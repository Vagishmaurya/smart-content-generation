/**
 * Main JavaScript for Mijbil the Otter interactive lesson
 */
...
function startModuleNarration(moduleId) {
    ...
    switch(moduleId) {
        case 'intro':
            if (window.narrator) {
                ...
                if (introModule) {
                    ...
                    let introContent = "Welcome to 'Mijbil the Otter'. ";
                    ...
                    window.narrator.speak("Welcome to the interactive English lesson on 'Mijbil the Otter'. This lesson explores the relationship between humans and animals through interactive activities.");
                }
            }
            break;
        ...
        case 'prereading':
            if (window.narrator) {
                ...
                if (prereadingModule) {
                    ...
                    // Fallback if module not found
                    window.narrator.speak("Welcome to the Let's Begin section. Here we'll explore the care that the author provided to Mijbil the otter and the emotions Mijbil displayed.");
                }
            }
            break;
        ...
        case 'thinking-language':
            if (window.narrator) {
                window.narrator.speak("Welcome to the Vocabulary and Grammar section. Here you'll explore vocabulary, noun modifiers, and participle forms from the texts.");
            }
            break;
            
        case 'activities':
            if (window.narrator) {
                window.narrator.speak("Welcome to the Activities section. Here you'll practice listening, speaking, and writing skills related to the themes of animal care, human-animal relationships, and wildlife conservation.");
            }
            break;
...
function startJourney() {
    ...
    showModule('story'); // Changed to redirect to 'The Texts' tab instead of 'prereading'
    if (narrator) {
        // Get the story content for a more complete narration
        const storyModule = document.getElementById('story');
        if (storyModule) {
            // Welcome message for the story section
            let storyIntroContent = "Welcome to the interactive English lesson on 'Mijbil the Otter'. Let's begin by reading the text about Mijbil and his adventures.";
            
            // Speak the welcome message
            narrator.speak(storyIntroContent);
            
            // The story narration will be handled by the story module's own functions
            // Automatically start reading the first part of the story after a delay
            setTimeout(() => {
                if (typeof readStoryPartAloud === 'function') {
                    readStoryPartAloud(1, false); // false = automatic call
                }
            }, 5000); // Wait 5 seconds after the intro message
        } else {
            // Fallback if module not found
            narrator.speak("Welcome to the interactive English lesson on 'Mijbil the Otter'. Let's begin by reading the text about Mijbil and his adventures.");
        }
    }
}
...
// Update score from other modules
...
function toggleReadAloud() {
    if (typeof readStoryPartAloud === 'function') {
        ...
        if (activePart) {
            ...
            const partNumber = partId === 'storyPart1' ? 1 : partId === 'storyPart2' ? 2 : 3;
