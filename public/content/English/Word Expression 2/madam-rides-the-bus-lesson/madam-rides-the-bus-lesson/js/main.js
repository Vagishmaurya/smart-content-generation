/**
 * Main JavaScript for Madam Rides the Bus interactive lesson
 */
...
function startModuleNarration(moduleId) {
    ...
    switch(moduleId) {
        case 'intro':
            if (window.narrator) {
                ...
                if (introModule) {
                    ...
                    let introContent = "Welcome to 'Madam Rides the Bus'. ";
                    ...
                    window.narrator.speak("Welcome to the interactive English lesson on 'Madam Rides the Bus'. This lesson explores a young girl's adventure and experiences on her first bus journey.");
                }
            }
            break;
        ...
        case 'prereading':
            if (window.narrator) {
                ...
                if (prereadingModule) {
                    ...
                    // Fallback if module not found
                    window.narrator.speak("Welcome to the Let's Begin section. Here we'll explore Valli's adventures on her first bus ride.");
                }
            }
            break;
        ...
        case 'thinking-text':
            if (window.narrator) {
                window.narrator.speak("Welcome to the Reading Comprehension section. Here you'll analyze the text and answer questions about it.");
            }
            break;
            
        case 'thinking-language':
            if (window.narrator) {
                window.narrator.speak("Welcome to the Vocabulary and Grammar section. Here you'll explore vocabulary, idioms, phrasal verbs and other language features from the text.");
            }
            break;
            
        case 'activities':
            if (window.narrator) {
                window.narrator.speak("Welcome to the Activities section. Here you'll practice listening, speaking, and writing skills related to the lesson.");
            }
            break;
...
function getModuleName(moduleId) {
    switch (moduleId) {
        ...
        case 'story': return 'Reading Comprehension Text';
        case 'thinking-text': return 'Reading Comprehension Questions';
...
function startJourney() {
    ...
    if (narrator) {
        ...
        if (prereadingModule) {
            const contentBlocks = prereadingModule.querySelectorAll('.content-block');
            let prereadingContent = "Welcome to the interactive English lesson on 'Madam Rides the Bus'. Let's begin by exploring Valli's journey on her first bus ride. ";
            ...
            // Fallback if module not found
            narrator.speak("Welcome to the interactive English lesson on 'Madam Rides the Bus'. Let's begin by exploring Valli's journey on her first bus ride.");
        }
    }
}
...
function recordSpeaking() {
    ...
    if (narrator) {
        narrator.speak("Please discuss the given role play scenario with your partner.");
    }
}

// Play listening activity
function playListeningActivity() {
    console.log('Playing Mizoram passage...');
    
    if (narrator) {
        // Get the Mizoram passage text
        const mizoramText = listeningActivity.passage;
        
        // Show reading indicator
        const readingIndicator = document.createElement('div');
        readingIndicator.className = 'reading-indicator';
        readingIndicator.innerHTML = `
            <div><span class="reading-spinner"></span> Reading aloud...</div>
            <button class="stop-narration-btn" onclick="stopNarration()">Stop Reading</button>
        `;
        
        const listeningSection = document.querySelector('.listening-activity');
        if (listeningSection) {
            listeningSection.appendChild(readingIndicator);
        }
        
        // Get all paragraphs
        const paragraphs = document.querySelectorAll('.mizoram-paragraph');
        if (paragraphs.length > 0) {
            // Clear any existing highlights
            paragraphs.forEach(p => p.classList.remove('paragraph-highlight'));
            
            // Create array to store timeouts for cleanup
            window.highlightTimeouts = [];
            
            // Read each paragraph sequentially
            let delay = 0;
            paragraphs.forEach((paragraph, i) => {
                ...
                const timeout = setTimeout(() => {
                    ...
                    // Remove indicator after last paragraph
                    if (i === paragraphs.length - 1) {
                        setTimeout(() => {
                            if (readingIndicator.parentNode) {
                                readingIndicator.parentNode.removeChild(readingIndicator);
                            }
                        }, text.length * 80); // Approximate time to finish reading
                    }
                }, delay);
                
                window.highlightTimeouts.push(timeout);
                delay += text.length * 80; // Approximate time based on text length
            });
        } else {
            // Fallback if paragraphs aren't found
            narrator.speak(mizoramText);
        }
    }
}

// Check answers for Mizoram listening activity
function checkMizoramAnswers() {
    let correctAnswers = 0;
    const totalQuestions = listeningActivity.questions.length;
    
    listeningActivity.questions.forEach((question, idx) => {
        const feedbackEl = document.getElementById(`mizoram-feedback-${idx}`);
        
        if (question.type === 'mcq') {
            // For multiple choice questions
            const selectedOption = document.querySelector(`input[name="mizoram-q${idx}"]:checked`);
            ...
            const answerEl = document.getElementById(`mizoram-answer-${idx}`);
            ...
            const keywords = expectedAnswer.split(',').map(word => word.trim());
            
            // Check if the answer contains at least some of the keywords
            ...
            if (matchedKeywords.length >= Math.ceil(keywords.length / 2)) {
                ...
                feedbackEl.textContent = 'Your answer could be improved. Consider including more details.';
                feedbackEl.className = 'question-feedback incorrect';
            }
        }
    });
    
    // If all answers correct, mark activity as completed
    if (correctAnswers === totalQuestions) {
        score += 20;
        document.getElementById('totalScore').textContent = score;
        showAchievement('Listening Activity Completed: All answers correct!');
    }
}
...
function showResource(resourceId) {
    ...
    if (narrator) {
        switch(resourceId) {
            case 'train-travel':
                narrator.speak("This guide provides tips for safe and comfortable train travel, including what to pack, how to secure tickets, and important safety information.");
                break;
            case 'bus-travel':
                narrator.speak("This resource offers guidelines for bus travel, including how to read bus schedules, understanding bus routes, and etiquette for bus journeys.");
                break;
            case 'project-template':
                narrator.speak("This template provides a structure for your project on different modes of transportation, including sections for preparation, safety measures, and travel experiences.");
                break;
        }
    }
}