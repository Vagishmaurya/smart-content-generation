/**
 * Main JavaScript for A Truly Beautiful Mind interactive lesson
 */
...
function startModuleNarration(moduleId) {
    ...
    switch(moduleId) {
        case 'intro':
            if (window.narrator) {
                ...
                if (introModule) {
                    ...
                    let introContent = "Welcome to 'A Truly Beautiful Mind'. ";
                    ...
                    window.narrator.speak("Welcome to the interactive English lesson on 'A Truly Beautiful Mind'. This lesson explores the life of Albert Einstein, one of the greatest scientific minds in history.");
                }
            }
            break;
        ...
        case 'prereading':
            if (window.narrator) {
                ...
                if (prereadingModule) {
                    ...
                    // Fallback if module not found
                    window.narrator.speak("Welcome to the Let's Begin section. Let's think about what makes a person a genius, and explore Albert Einstein's remarkable achievements.");
                }
            }
            break;
        ...
        case 'thinking-language':
            if (window.narrator) {
                window.narrator.speak("Welcome to the Vocabulary and Grammar section. Here you'll explore participial phrases and vocabulary from the story.");
            }
            break;
...
function showQuestionSet(setId) {
    ...
    const activeNavButton = document.querySelector(`.story-nav-btn[onclick="showQuestionSet('${setId}')"]`);
    if (activeNavButton) {
...
function loadStoryQuestions() {
    if (typeof storyQuestions !== 'undefined') {
        const questionsContainer = document.getElementById('textQuestions');
        questionsContainer.innerHTML = '';
        
        storyQuestions.forEach((section, sIndex) => {
            const sectionElement = document.createElement('div');
            sectionElement.className = 'question-group';
            
            const sectionTitle = document.createElement('h3');
            sectionTitle.className = 'question-group-title';
            ...
            section.questions.forEach(question => {
                ...
                answerArea.className = 'question-answers';
                
                if (question.type === 'short' || question.type === 'reference') {
                    const textarea = document.createElement('textarea');
                    textarea.rows = 3;
                    textarea.id = question.id;
                    textarea.placeholder = 'Write your answer here...';
                    textarea.setAttribute('data-answer', question.answer);
                    ...
                    select.setAttribute('data-answer', question.correctOption);
        ...
        checkButton.onclick = checkStoryAnswers;
        ...
        feedbackArea.id = 'questionsFeedback';
        feedbackArea.className = 'feedback-message';
        feedbackArea.setAttribute('aria-live', 'polite');
        questionsContainer.appendChild(feedbackArea);
    }
}

function loadPoemQuestions() {
    if (typeof poemQuestions !== 'undefined') {
        const questionsContainer = document.getElementById('textQuestions');
        questionsContainer.innerHTML = '';
        
        poemQuestions.forEach((section, sIndex) => {
            const sectionElement = document.createElement('div');
            sectionElement.className = 'question-group';
        ...
        checkButton.className = 'interactive-btn';
        checkButton.textContent = 'Check Answers';
        checkButton.onclick = checkPoemAnswers;
...
function startJourney() {
    ...
    if (narrator) {
        ...
        if (prereadingModule) {
            const contentBlocks = prereadingModule.querySelectorAll('.content-block');
            let prereadingContent = "Welcome to the interactive English lesson on 'A Truly Beautiful Mind'. Let's begin by exploring what you think about genius. ";
            ...
            // Fallback if module not found
            narrator.speak("Welcome to the interactive English lesson on 'A Truly Beautiful Mind'. Let's begin by exploring what you think about genius.");
        }
    }
}
