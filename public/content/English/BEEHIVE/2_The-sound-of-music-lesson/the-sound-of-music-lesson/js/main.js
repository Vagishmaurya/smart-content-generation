/**
 * Main JavaScript for The Sound of Music interactive lesson
 */
...
window.addEventListener('load', () => {
    ...
    // Add global user interaction tracking WITHOUT once:true to ensure it works with all interactions
    ...
    // Force userInteracted to true after a short delay if not already set
    // This is a fallback to ensure narration works
    setTimeout(() => {
    if (!userInteracted) {
            console.log('🔄 Forcing userInteracted to true after timeout');
        userInteracted = true;
        }
    }, 3000);
});
...
function initializeNarrator() {
    if ('speechSynthesis' in window) {
        ...
        narrator = {
            ...
            speak: function(text) {
                console.log('🎯 speak() called with text length:', text.length, 'enabled:', this.enabled, 'userInteracted:', userInteracted);
                if (!this.enabled) {
                    console.log('❌ Narrator is disabled, not speaking');
                    return;
                }
                ...
                // Force userInteracted to true - we're assuming the user has interacted by now
                // This is a workaround for the autoplay policy
                if (!userInteracted) {
                    console.log('⚠️ Forcing userInteracted to true for speech synthesis');
                    userInteracted = true;
                }
                
                console.log('✅ User has interacted, proceeding with speech');
            ...
            showFallbackMessage: function(text) {
                ...
                // Try again after a short delay - the user might have interacted by then
                setTimeout(() => {
                    console.log("🔄 Checking if user has interacted since fallback message:", userInteracted);
                    if (userInteracted && this.enabled) {
                        console.log("🔄 User has interacted, retrying speech");
                        this.speak(text);
                    }
                }, 1000);
            }
...
function showModule(moduleId) {
    ...
    // Track user interaction when navigating modules
    console.log('🔍 showModule called for:', moduleId, 'tracking user interaction');
    ...
    switch(moduleId) {
        case 'story':
            // Load the story content if needed
            const storyContentDiv = document.getElementById('storyContent');
            if (storyContentDiv && typeof fullStoryContent === 'undefined') {
                // Try to combine all story content parts if they exist
                if (typeof storyContentPart1 !== 'undefined' && 
                    typeof storyContentPart2 !== 'undefined' && 
                    typeof storyContentPart3 !== 'undefined' && 
                    typeof storyContentPart4 !== 'undefined') {
                    
                    const combinedContent = storyContentPart1 + storyContentPart2 + storyContentPart3 + storyContentPart4;
                    storyContentDiv.innerHTML = combinedContent;
                } else {
                    console.error('Story content parts not found');
                }
            }
            break;
        case 'poem':
...
function startModuleNarration(moduleId) {
    console.log('🎙️ startModuleNarration called for:', moduleId, 'userInteracted:', userInteracted);
    if (!window.narrator || !window.narrator.enabled) {
        console.log('❌ Narrator not available or disabled');
        return;
    }
    console.log('✅ Narrator is available and enabled');
    ...
    switch(moduleId) {
        case 'intro':
            if (window.narrator) {
                ...
                if (introModule) {
                    ...
                    let introContent = "Welcome to 'The Sound of Music'. ";
                    ...
                    window.narrator.speak("Welcome to the interactive English lesson on 'The Sound of Music'. This chapter explores the lives of two remarkable musicians - Evelyn Glennie and Ustad Bismillah Khan.");
                }
            }
            break;
        ...
        case 'prereading':
            if (window.narrator) {
                ...
                if (prereadingModule) {
                    ...
                    // Fallback if module not found
                    window.narrator.speak("Welcome to the Let's Begin section. Let's think about people who have overcome significant challenges in their lives.");
                }
            }
            break;
        ...
        case 'thinking-text':
            if (window.narrator) {
                window.narrator.speak("Welcome to the Reading Comprehension section. Here you'll analyze the story and poem and answer questions about them.");
            }
            break;
            
        case 'thinking-language':
            if (window.narrator) {
                window.narrator.speak("Welcome to the Vocabulary and Grammar section. Here you'll explore words and their usage from the story.");
            }
            break;
...
function trackUserInteraction() {
    console.log('🎯 trackUserInteraction() called, current state:', userInteracted);
    if (!userInteracted) {
        userInteracted = true;
        console.log('✅ User interaction detected - speech synthesis enabled');
        
        // Force a silent sound to kickstart speech synthesis
        if ('speechSynthesis' in window) {
            console.log('🔊 Playing silent sound to initialize speech synthesis');
...
function startJourney() {
    ...
    // Note: We don't need to start narration here because showModule already calls startModuleNarration
}
