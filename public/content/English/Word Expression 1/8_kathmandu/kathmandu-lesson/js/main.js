/**
 * Main JavaScript for Kathmandu interactive lesson
 */
...
function startModuleNarration(moduleId) {
    ...
    switch(moduleId) {
        case 'intro':
            if (window.narrator) {
                ...
                if (introModule) {
                    ...
                    let introContent = "Welcome to 'Kathmandu'. ";
                    ...
                    window.narrator.speak("Welcome to the interactive English lesson on 'Kathmandu'. This lesson explores Vikram Seth's travelogue describing his journey through Kathmandu.");
                }
            }
            break;
        ...
        case 'prereading':
            if (window.narrator) {
                ...
                if (prereadingModule) {
                    ...
                    // Fallback if module not found
                    window.narrator.speak("Welcome to the Let's Begin section. Here we'll discuss the travelogue of Vikram Seth and an interesting fact about a river in New Zealand.");
                }
            }
            break;
        ...
        case 'thinking-language':
            if (window.narrator) {
                window.narrator.speak("Welcome to the Vocabulary and Grammar section. Here you'll explore vocabulary, phrasal verbs, suffixes, and grammar from the texts.");
            }
            break;
...
function startJourney() {
    ...
    if (narrator) {
        ...
        if (prereadingModule) {
            const contentBlocks = prereadingModule.querySelectorAll('.content-block');
            let prereadingContent = "Welcome to the interactive English lesson on 'Kathmandu'. Let's begin by exploring Vikram Seth's travelogue. ";
            ...
            // Fallback if module not found
            narrator.speak("Welcome to the interactive English lesson on 'Kathmandu'. Let's begin by exploring Vikram Seth's travelogue.");
        }
    }
}
...
function toggleReadAloud() {
    // This function will be defined in kathmandu-story.js
    if (typeof readStoryPartAloud === 'function') {
        // Find which part is currently active
        const activePart = document.querySelector('.story-nav-btn.active');
        if (activePart) {
            const partIndex = Array.from(document.querySelectorAll('.story-nav-btn')).indexOf(activePart) + 1;
...
// Check suffix removal
function checkSuffixRemoval() {
    let correct = 0;
    const totalQuestions = document.querySelectorAll('.suffix-input').length;
    
    document.querySelectorAll('.suffix-input').forEach(input => {
    ...
    const feedbackEl = document.getElementById('suffixRemovalFeedback');
    feedbackEl.textContent = `You got ${correct} out of ${totalQuestions} correct!`;
    feedbackEl.className = 'feedback-message show ' + (correct === totalQuestions ? 'success' : 'warning');
    ...
    if (narrator) {
        narrator.speak(`You got ${correct} out of ${totalQuestions} correct!`);
    }
}

// Check suffix addition
function checkSuffixAddition() {
    let correct = 0;
    const totalQuestions = document.querySelectorAll('.suffix-input-2').length;
    
    document.querySelectorAll('.suffix-input-2').forEach(input => {
        const userAnswer = input.value.trim().toLowerCase();
        const correctAnswer = input.getAttribute('data-answer').toLowerCase();
    ...
    const feedbackEl = document.getElementById('suffixAdditionFeedback');
    feedbackEl.textContent = `You got ${correct} out of ${totalQuestions} correct!`;
    feedbackEl.className = 'feedback-message show ' + (correct === totalQuestions ? 'success' : 'warning');
...
// Save tense analysis
function saveTenseAnalysis() {
    const analysisText = document.getElementById('tenseAnalysis').value.trim();
    
    if (analysisText.length < 30) {
        const feedbackEl = document.getElementById('tenseAnalysisFeedback');
        feedbackEl.textContent = 'Please write a more detailed analysis (at least 30 characters).';
        feedbackEl.className = 'feedback-message show error';
        return;
    }
    
    // Save the analysis (in a real app, this would be sent to a server)
    const feedbackEl = document.getElementById('tenseAnalysisFeedback');
    feedbackEl.textContent = 'Your analysis has been saved!';
    feedbackEl.className = 'feedback-message show success';
    ...
    if (!modulesCompleted.includes('thinking-language')) {
        ...
        updateProgress();
        showAchievement('Grammar Analysis Completed!');
    }
    
    if (narrator) {
        narrator.speak("Thank you for your analysis. Your work has been saved.");
    }
}

// Save punctuation
function savePunctuation() {
    const punctuationText = document.getElementById('punctuationExercise').value.trim();
    
    if (punctuationText.length < 50) {
        alert('Please provide your punctuated version of the text (at least 50 characters).');
        return;
    }
    
    // Check if the text includes some punctuation marks
    if (!punctuationText.includes('.') && !punctuationText.includes(',') && !punctuationText.includes(';')) {
        alert('Please add punctuation marks to the text!');
        return;
    }
    
    // Save the punctuated text (in a real app, this would be sent to a server)
    const feedbackEl = document.getElementById('punctuationFeedback');
    feedbackEl.textContent = 'Your punctuated text has been saved!';
    feedbackEl.className = 'feedback-message show success';
    ...
    if (narrator) {
        narrator.speak("Your punctuated text has been saved. Good job!");
    }
}
...
function saveListeningAnswers() {
    ...
    const answerTextareas = document.querySelectorAll('.listening-questions textarea');
    let answeredCount = 0;
...
function saveSpeech() {
    ...
    if (speechText.length < 100) {
        alert('Please write a more detailed speech (at least 100 characters).');
        return;
    }
    
    // Save the speech (in a real app, this would be sent to a server)
    alert('Your speech has been saved!');
    ...
    if (narrator) {
        narrator.speak("Excellent work on your speech!");
    }
}
    ...
    if (narrator) {
        switch(resourceId) {
            case 'music-instruments':
                narrator.speak("This guide provides information about various musical instruments used across different cultures and genres.");
                break;
            case 'travel-planning':
                narrator.speak("The travel planning template helps organize your itinerary, including transportation, accommodations, activities, and budgeting.");
                break;
            case 'northeast-map':
                narrator.speak("This interactive map of Northeast India shows major cities, landmarks, and transportation routes for planning your journey.");
                break;
        }
    }
}
